...
async function resolvePin(pin) {
    return new Promise(r => {
        ...
        k.on("Joined", () => { ok=true; clearTimeout(to); r({uuid: k.quiz?.uuid, title: k.quiz?.name}); cl(); });
...
client.on('interactionCreate', async it => {
    ...
    if (it.isButton()) {
        ...
        if (it.customId.startsWith('raid_')) {
            ...
            if (!r) return it.reply({content:"❌ Raid dead.", ephemeral:true});
            const map = {tri:0, dia:1, cir:2, squ:3};
            if (map[act] !== undefined) {
                r.bots.forEach(b => b.socket && b.answer(map[act]));
                it.reply({content:`🔫 Fire: ${act}`, ephemeral:true});
            }
        }
        return;
    }
    ...
    if (commandName === 'kahoot') {
        ...
        if (sub === 'scan') {
            ...
            try {
                ...
                else it.editReply("❌ No UUID found.");
        ...
        if (sub === 'resolve') {
            ...
            else it.editReply("❌ Failed to resolve.");
        }
        ...
        if (sub === 'injector') {
            ...
            const embed = new EmbedBuilder().setColor(0x8b5cf6).setTitle("💉 INJECTION PAYLOAD")
                .setDescription("Copy this into Console (F12) or create a Bookmarklet.")
        ...
        if (sub === 'ping') {
            ...
            const embed = new EmbedBuilder().setColor(0x3b82f6).setTitle("📡 SYSTEM STATUS")
                .addFields(
                    ...
                    {name: 'RAM (Active)', value: `${(mem.active/1024/1024).toFixed(0)}MB`, inline:true}
                );
...
app.listen(PORT, () => console.log(`🌍 V6 ONLINE ${PORT}`));
